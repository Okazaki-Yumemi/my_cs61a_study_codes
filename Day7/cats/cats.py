...
def pick(paragraphs, select, k):
    ...
    # BEGIN PROBLEM 1
    "*** YOUR CODE HERE ***"
    ok_list=[char for char in paragraphs if select(char)]
    if k>=len(ok_list):
        return ""
    return ok_list[k]
...
def autocorrect(typed_word, word_list, diff_function, limit):
    ...
    if typed_word in word_list:
        ...
        for i in range(0,len(word_list)):
            ...
            if diff<=limit:
                touple1=((word_list[i]),diff,i)
                solu.append(touple1)
        solu=sorted(solu,key=lambda x:x[2])
        solu=sorted(solu,key=lambda x:x[1])
        if solu:
            return solu[0][0]
        else:
            return typed_word
...
def report_progress(typed, source, user_id, upload):
    ...
    sum=len(source)
    count=0
    def judge_fornt(typed, source,n):
        judge=True
        for i in range(0,n):
            if typed[i] !=source[i]:
                judge=False
...
def time_per_word(words, timestamps_per_player):
    ...
    def get_list(list):
        ...
        return new_list
    times = [get_list(tpp[i]) for i in range(0,len(tpp))]  # You may remove this line
    # END PROBLEM 9
...
def fastest_words(words_and_times):
    ...
    # BEGIN PROBLEM 10
    "*** YOUR CODE HERE ***"
    return_list=[[] for i in range(0,len(times))]
    def find_smallest(list):
        for i in range(0,len(list)):
            if min(list)==list[i]:
                return i
    for i in range(0,len(words)):
        judge_list=[ list[i] for list in times]
        return_list[find_smallest(judge_list)].append(words[i])
    return return_list
    # END PROBLEM 10
