def draw(hand, positions):
    ...
    ['A', 'J', 9]
    """
    return list(reversed( [hand.pop(i) for i in list(sorted(positions))] ))
...
class Keyboard:
    ...
    def __init__(self):
        self.typed = []
        self.keys = {c: Button(c, lambda ch: self.typed.append(ch)) for c in LOWERCASE_LETTERS}  # Try a dictionary comprehension!
