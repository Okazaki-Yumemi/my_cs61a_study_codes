    ...
    def add_to(self, place):
        if place.ant is None:
            ...
            if place.ant.can_contain(self) ==True:
                ...
                assert place.ant is None,"Too many ants in "+place.name
            # END Problem 8b
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self):
        ...
        distance=0
        #改变位置
        while current_place.is_hive==False and current_place is not None :
...
class FireAnt(Ant):
    ...
    def reduce_health(self, amount):
        ...
        if self.health<=0:
            reflected=reflected+self.damage
...
class HungryAnt(Ant):
    ...
    def action(self, gamestate):
        if self.place.bees and self.cooldown==0:
            bee1=random_bee(self.place.bees)
            bee1.reduce_health(bee1.health)
...
class QueenAnt(ThrowerAnt):
    ...
    def __init__(self, health=1):
        super().__init__(health)
        if len(QueenAnt.instances) > 0:
            self.impostor = True
        else:
            self.impostor = False
            QueenAnt.instances.append(self)
    ...
    def action(self, gamestate):
        ...
        "*** YOUR CODE HERE ***"
        if self.impostor:
            self.reduce_health(self.health) # 冒牌货当场去世
            return
        ...
        while current_place is not None:
            ...
            if ant is not None:
                if ant.is_container and ant.ant_contained is not None:
                    if ant.ant_contained.already_doubled == False:
                        ant.ant_contained.double()
                if ant.already_doubled == False:
                    ant.double()
            current_place = current_place.exit
